 ...
 */
static void
apc_core_unmap(int signo, siginfo_t* siginfo, void* context)
{
    TSRMLS_FETCH();
...
/* {{{ apc_reload_cache */
static void
apc_clear_cache(int signo, siginfo_t* siginfo, void* context)
{
    TSRMLS_FETCH();
 ...
 */
static void
apc_rehandle_signal(int signo, siginfo_t* siginfo, void* context)
{
    int i;
 ...
 */
static int
apc_register_signal(int signo, void (*handler)(int, siginfo_t*, void*))
{
    ...
    if (sigaction(signo, NULL, &sa) == 0) {
        ...
        if (sa.sa_handler != SIG_ERR && sa.sa_handler != SIG_DFL && sa.sa_handler != SIG_IGN) {
            ...
            apc_signal_info.prev = (apc_signal_entry_t**) apc_erealloc(
              apc_signal_info.prev, (apc_signal_info.installed + 1) * sizeof(apc_signal_entry_t*));
            apc_signal_info.prev[apc_signal_info.installed] =
              (apc_signal_entry_t*) apc_emalloc(sizeof(apc_signal_entry_t));
