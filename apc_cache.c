...
/* {{{ make_slot */
apc_cache_slot_t*
make_slot(apc_cache_t* cache, apc_cache_key_t* key, apc_cache_entry_t* value, apc_cache_slot_t* next, time_t t)
{
...
/* {{{ free_slot */
static void
free_slot(apc_cache_slot_t* slot)
{
    /* destroy slot pool */
...
static void
apc_cache_hash_slot(apc_cache_t* cache, zend_string* key, zend_ulong* hash, zend_ulong* slot)
{
    (*hash) = ZSTR_HASH(key);
...
PHP_APCU_API void
apc_cache_remove_slot(apc_cache_t* cache, apc_cache_slot_t** slot)
{
...
/* {{{ apc_cache_gc */
PHP_APCU_API void
apc_cache_gc(apc_cache_t* cache)
{
...

    {
        ...
        while (*slot != NULL) {
            ...
            if (!(*slot)->value->ref_count || gc_sec > (time_t) cache->gc_ttl) {
                ...
                /* good ol' whining */
                if (dead->value->ref_count > 0) {
                    apc_debug("GC cache entry '%s' was on gc-list for %d seconds", dead->key.str, gc_sec);
                }
...
PHP_APCU_API int APC_UNSERIALIZER_NAME(php)(APC_UNSERIALIZER_ARGS)
{
    ...
    if (!php_var_unserialize(value, &tmp, buf + buf_len, &var_hash)) {
        ...
        php_error_docref(NULL, E_NOTICE, "Error at offset %ld of %ld bytes", (zend_long)(tmp - buf),
                         (zend_long) buf_len);
        ZVAL_NULL(value);
        return 0;
    }
...
apc_cache_create(apc_sma_t* sma,
                 ...
                 zend_bool defend)
{
    ...
    if (!cache->shmaddr) {
        apc_error(
          "Unable to allocate shared memory for cache structures.  (Perhaps your shared memory size isn't large "
          "enough?). ");
        return NULL;
    }
...
apc_cache_insert_internal(apc_cache_t* cache,
                          apc_cache_key_t* key,
...
apc_cache_store_internal(
  apc_cache_t* cache, zend_string* strkey, const zval* val, const int32_t ttl, const zend_bool exclusive)
{
...
static inline apc_cache_entry_t*
apc_cache_find_internal(apc_cache_t* cache, zend_string* key, time_t t, zend_bool lock)
{
    apc_cache_slot_t** slot;
    zend_ulong h, s;
...
static inline zend_bool
apc_cache_fetch_internal(apc_cache_t* cache, zend_string* key, apc_cache_entry_t* entry, time_t t, zval** dst)
{
...
apc_cache_store(apc_cache_t* cache, zend_string* strkey, const zval* val, const int32_t ttl, const zend_bool exclusive)
{
    apc_cache_entry_t* entry;
    apc_cache_key_t key;
    time_t t;
...
/* {{{ data_unserialize */
static zval
data_unserialize(const char* filename)
{
    zval retval;
    zend_long len = 0;
    zend_stat_t sb;
    ...
    if (!php_var_unserialize(&retval, (const unsigned char**) &tmp, (const unsigned char*) (contents + len),
                             &var_hash)) {
        fclose(fp);
        free(contents);
        return EG(uninitialized_zval);
...
static int
apc_load_data(apc_cache_t* cache, const char* data_file)
{
    char* p;
    char key[MAXPATHLEN] = {
        0,
    ...
    if (p && p[1]) {
        ...
        if (p) {
            ...
            if (Z_TYPE(data) != IS_UNDEF) {
                ...
                apc_cache_store(cache, name, &data, 0, 1);
...
PHP_APCU_API zend_bool
apc_cache_preload(apc_cache_t* cache, const char* path)
{
#ifndef ZTS
    zend_bool result      = 0;
    ...
    if ((ndir = php_scandir(path, &namelist, 0, php_alphasort)) > 0) {
        for (i = 0; i < ndir; i++) {
            /* check for extension */
            if (!(p = strrchr(namelist[i]->d_name, '.')) || (p && strcmp(p, ".data"))) {
                free(namelist[i]);
                continue;
            }
...
/* {{{ apc_cache_release */
PHP_APCU_API void
apc_cache_release(apc_cache_t* cache, apc_cache_entry_t* entry)
{
...
/* {{{ apc_cache_destroy */
PHP_APCU_API void
apc_cache_destroy(apc_cache_t* cache)
{
    if (!cache) {
        return;
    }
...
PHP_APCU_API void
apc_cache_real_expunge(apc_cache_t* cache)
{
    /* increment counter */
...
/* {{{ apc_cache_clear */
PHP_APCU_API void
apc_cache_clear(apc_cache_t* cache)
{
...
PHP_APCU_API void
apc_cache_default_expunge(apc_cache_t* cache, size_t size)
{
    time_t t;
    size_t suitable  = 0L;
    size_t available = 0L;
...
PHP_APCU_API zend_bool
apc_cache_make_context(apc_cache_t* cache,
                       ...
                       uint force_update)
{
    ...
    case APC_CONTEXT_SHARE: {
        return apc_cache_make_context_ex(context, cache->serializer, (apc_malloc_t) cache->sma->smalloc,
                                         cache->sma->sfree, cache->sma->protect, cache->sma->unprotect, pool_type,
                                         copy_type, force_update);
    } break;

    case APC_CONTEXT_NOSHARE: {
        return apc_cache_make_context_ex(context, cache->serializer, apc_php_malloc, apc_php_free, NULL, NULL,
...
PHP_APCU_API zend_bool
apc_cache_make_context_ex(apc_context_t* context,
                          apc_serializer_t* serializer,
                          ...
                          uint force_update)
{
    ...
    context->pool = apc_pool_create(pool_type, _malloc, _free, _protect, _unprotect);
...
/* {{{ apc_context_destroy */
PHP_APCU_API zend_bool
apc_cache_destroy_context(apc_context_t* context)
{
    if (!context->pool) {
        return 0;
    }
...
/* {{{ apc_cache_insert */
PHP_APCU_API zend_bool
apc_cache_insert(apc_cache_t* cache,
                 apc_cache_key_t* key,
                 apc_cache_entry_t* value,
                 apc_context_t* ctxt,
                 time_t t,
                 zend_bool exclusive)
{
    ...
    php_apc_try(APC_LOCK(cache->header), { result = apc_cache_insert_internal(cache, key, value, ctxt, t, exclusive); },
                APC_UNLOCK(cache->header));
...
apc_cache_find(apc_cache_t* cache, zend_string* key, time_t t)
{
...
/* {{{ apc_cache_fetch */
PHP_APCU_API zend_bool
apc_cache_fetch(apc_cache_t* cache, zend_string* key, time_t t, zval** dst)
{
    apc_cache_entry_t* entry = NULL;
    zend_bool ret            = 0;
...
/* {{{ apc_cache_exists */
PHP_APCU_API apc_cache_entry_t*
apc_cache_exists(apc_cache_t* cache, zend_string* key, time_t t)
{
...
/* {{{ apc_cache_update */
PHP_APCU_API zend_bool
apc_cache_update(apc_cache_t* cache, zend_string* key, apc_cache_updater_t updater, void* data)
{
    apc_cache_slot_t** slot;
...
/* {{{ apc_cache_delete */
PHP_APCU_API zend_bool
apc_cache_delete(apc_cache_t* cache, zend_string* key)
{
    apc_cache_slot_t** slot;
...
/* {{{ apc_cache_make_key */
PHP_APCU_API zend_bool
apc_cache_make_key(apc_cache_key_t* key, zend_string* str)
{
    assert(key != NULL);
...
/* {{{ my_serialize_object */
static zval*
my_serialize_object(zval* dst, const zval* src, apc_context_t* ctxt)
{
    ...
    if (serialize((unsigned char**) &buf, &buf_len, src, config)) {
        ...
        /* The only necessary one is probably IS_TYPE_REFCOUNTED - (We check Z_REFCOUNTED_P when de-duplicating
         * serialized values. */
...
static zval*
my_unserialize_object(zval* dst, const zval* src, apc_context_t* ctxt)
{
...
apc_array_dup_element(apc_context_t* ctxt,
                      HashTable* source,
                      HashTable* target,
                      uint32_t idx,
                      Bucket* p,
    ...
    do {
        if (Z_OPT_REFCOUNTED_P(data)) {
            if (Z_ISREF_P(data) && Z_REFCOUNT_P(data) == 1 &&
                (Z_TYPE_P(Z_REFVAL_P(data)) != IS_ARRAY || Z_ARRVAL_P(Z_REFVAL_P(data)) != source)) {
...
apc_array_dup_packed_elements(apc_context_t* ctxt, HashTable* source, HashTable* target, int with_holes)
{
...
apc_array_dup_elements(apc_context_t* ctxt, HashTable* source, HashTable* target, int static_keys, int with_holes)
{
    uint32_t idx = 0;
    Bucket* p    = source->arData;
    Bucket* q    = target->arData;
...
static APC_HOTSPOT HashTable*
my_copy_hashtable(HashTable* source, apc_context_t* ctxt)
{
    uint32_t idx;
    HashTable* target;
    apc_pool* pool = ctxt->pool;
    ...
    if (source->nNumUsed == 0) {
        target->u.flags = (source->u.flags & ~(HASH_FLAG_INITIALIZED | HASH_FLAG_PACKED | HASH_FLAG_PERSISTENT)) |
                          HASH_FLAG_APPLY_PROTECTION | HASH_FLAG_STATIC_KEYS;
        ...
        if (target->nNumOfElements > 0 && target->nInternalPointer == HT_INVALID_IDX) {
            idx = 0;
...
static APC_HOTSPOT zend_reference*
my_copy_reference(const zend_reference* src, apc_context_t* ctxt)
{
    apc_pool* pool = ctxt->pool;
    zend_reference* dst;
...
/* {{{ my_copy_zval */
/* This function initializes *dst (temporary zval with request lifetime) with the (possibly serialized) contents of the
 * serialized value in *src */
static APC_HOTSPOT zval*
my_copy_zval(zval* dst, const zval* src, apc_context_t* ctxt)
{
    apc_pool* pool = ctxt->pool;
    ...
    if (Z_REFCOUNTED_P(src)) {
        if (zend_hash_num_elements(&ctxt->copied)) {
            zval* rc = zend_hash_index_find(&ctxt->copied, (uintptr_t) Z_COUNTED_P(src));
            if (rc) {
    ...
    case IS_OBJECT:
        if (ctxt->copy == APC_COPY_IN) {
            /* For objects and arrays, their pointer points to a serialized string instead of a zend_array or
             * zend_object. */
            /* Unserialize that, and on success, give dst the default type flags for an object/array (We check
             * Z_REFCOUNTED_P below). */
...
/* {{{ apc_copy_zval */
PHP_APCU_API zval*
apc_copy_zval(zval* dst, const zval* src, apc_context_t* ctxt)
{
...
PHP_APCU_API zval*
apc_cache_store_zval(zval* dst, const zval* src, apc_context_t* ctxt)
{
...
PHP_APCU_API zval*
apc_cache_fetch_zval(apc_context_t* ctxt, zval* dst, const zval* src)
{
    if (Z_TYPE_P(src) == IS_ARRAY) {
        /* Maintain a list of zvals we've copied to properly handle recursive structures */
        zend_hash_init(&ctxt->copied, 16, NULL, NULL, 0);
...
/* {{{ apc_cache_link_info */
static zval
apc_cache_link_info(apc_cache_t* cache, apc_cache_slot_t* p)
{
    zval link;
...
/* {{{ apc_cache_info */
PHP_APCU_API zval
apc_cache_info(apc_cache_t* cache, zend_bool limited)
{
    zval info;
    zval list;
    zval gc;
...
*/
PHP_APCU_API zval*
apc_cache_stat(apc_cache_t* cache, zend_string* key, zval* stat)
{
    apc_cache_slot_t** slot;
    zend_ulong h, s;
...
/* {{{ apc_cache_busy */
PHP_APCU_API zend_bool
apc_cache_busy(apc_cache_t* cache)
{
...
/* {{{ apc_cache_defense */
PHP_APCU_API zend_bool
apc_cache_defense(apc_cache_t* cache, apc_cache_key_t* key)
{
    zend_bool result = 0;
    ...
    if (cache->defend) {
        ...
        if (ZSTR_HASH(last->str) == ZSTR_HASH(key->str) && ZSTR_LEN(last->str) == ZSTR_LEN(key->str)) {
            ...
            if (last->mtime == key->mtime && FROM_DIFFERENT_THREAD(last)) {
                ...
                apc_debug("Potential cache slam averted for key '%s'", key->str);
                result = 1;
            } else {
...
PHP_APCU_API void
apc_cache_serializer(apc_cache_t* cache, const char* name)
{
...
PHP_APCU_API void
apc_cache_entry(apc_cache_t* cache,
                zval* key,
                zend_fcall_info* fci,
    ...
    php_apc_try(apc_cache_entry_try_begin(),
                {
                    ...
                    if (!entry) {
                        ...
                        if (result == SUCCESS) {
                            ...
                            if (!EG(exception)) {
                                apc_cache_store_internal(cache, Z_STR_P(key), return_value, (uint32_t) ttl, 1);
                            }
                        }
                    } else
