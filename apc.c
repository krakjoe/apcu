...
PHP_APCU_API char* APC_ALLOC
apc_estrdup(const char* s)
{
    int len;
    char* dup;
...
apc_xstrdup(const char* s, apc_malloc_t f)
{
    return s != NULL ? apc_xmemcpy(s, strlen(s) + 1, f) : NULL;
}

PHP_APCU_API void* APC_ALLOC
apc_xmemcpy(const void* p, size_t n, apc_malloc_t f)
{
    void* q;
...
char*
apc_append(const char* s, const char* t)
{
    int slen;
    int tlen;
    char* p;
...
char*
apc_substr(const char* s, int start, int length)
{
    char* substr;
...
char**
apc_tokenize(const char* s, char delim)
{
...
unsigned int
apc_crc32(const unsigned char* buf, unsigned int len)
{
    unsigned int i;
    int k;
    unsigned int crc;
...
static apc_serializer_t apc_serializers[APC_MAX_SERIALIZERS] = { {
  0,
} };
/* }}} */
...
PHP_APCU_API int
_apc_register_serializer(const char* name, apc_serialize_t serialize, apc_unserialize_t unserialize, void* config)
{
    int i;
...
/* {{{ apc_find_serializer */
PHP_APCU_API apc_serializer_t*
apc_find_serializer(const char* name)
{
    int i;
    apc_serializer_t* serializer;
