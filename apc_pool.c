...
/* {{{ apc_pool_create */
PHP_APCU_API apc_pool*
apc_pool_create(apc_pool_type pool_type,
...
/* {{{ apc_pool_destroy */
PHP_APCU_API void
apc_pool_destroy(apc_pool* pool)
{
...
static void*
apc_unpool_alloc(apc_pool* pool, size_t size)
{
...
static void
apc_unpool_free(apc_pool* pool, void* ptr)
{
    apc_unpool* upool = (apc_unpool*) pool;
...
static void
apc_unpool_cleanup(apc_pool* pool)
{
}

static apc_pool*
apc_unpool_create(
  apc_pool_type type, apc_malloc_t allocate, apc_free_t deallocate, apc_protect_t protect, apc_unprotect_t unprotect)
{
...
/* {{{ redzone code */
static const unsigned char decaff[] = { 0xde, 0xca, 0xff, 0xc0, 0xff, 0xee, 0xba, 0xad, 0xde, 0xca, 0xff,
                                        0xc0, 0xff, 0xee, 0xba, 0xad, 0xde, 0xca, 0xff, 0xc0, 0xff, 0xee,
                                        0xba, 0xad, 0xde, 0xca, 0xff, 0xc0, 0xff, 0xee, 0xba, 0xad };

/* a redzone is at least 4 (0xde,0xca,0xc0,0xff) bytes */
#define REDZONE_SIZE(size)                                                                                             \
    ((ALIGNWORD((size)) > ((size) + 4)) ? (ALIGNWORD((size)) - (size)) : /* does not change realsize */                \
       ALIGNWORD((size)) - (size) + ALIGNWORD((sizeof(char))))           /* adds 1 word to realsize */

#define SIZEINFO_SIZE ALIGNWORD(sizeof(size_t))

#define MARK_REDZONE(block, redsize)                                                                                   \
    do {                                                                                                               \
...
#define INIT_POOL_BLOCK(rpool, entry, size)                                                                            \
    do {                                                                                                               \
...
/* {{{ create_pool_block */
static pool_block*
create_pool_block(apc_realpool* rpool, size_t size)
{
...
/* {{{ apc_realpool_alloc */
static void*
apc_realpool_alloc(apc_pool* pool, size_t size)
{
 ...
 */
static APC_USED int
apc_realpool_check_integrity(apc_realpool* rpool)
{
    ...
    if (!APC_POOL_HAS_REDZONES(pool) || !APC_POOL_HAS_SIZEINFO(pool)) {
 ...
 */
static void
apc_realpool_free(apc_pool* pool, void* p)
{
}
/* }}} */
...
static void
apc_realpool_cleanup(apc_pool* pool)
{
    pool_block* entry;
    pool_block* tmp;
    apc_realpool* rpool   = (apc_realpool*) pool;
...
/* {{{ apc_pstrdup */
PHP_APCU_API void* APC_ALLOC
apc_pstrdup(const char* s, apc_pool* pool)
{
...
/* {{{ apc_pmemcpy */
PHP_APCU_API void* APC_ALLOC
apc_pmemcpy(const void* p, size_t n, apc_pool* pool)
{
    void* q;
...
/* {{{ apc_pstrcpy */
PHP_APCU_API zend_string*
apc_pstrcpy(zend_string* str, apc_pool* pool)
{
    zend_string* p = (zend_string*) pool->palloc(pool, ZEND_MM_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(ZSTR_LEN(str))));
...
/* {{{ apc_pstrnew */
PHP_APCU_API zend_string*
apc_pstrnew(unsigned char* buf, size_t buf_len, apc_pool* pool)
{
    zend_string* p = (zend_string*) pool->palloc(pool, ZEND_MM_ALIGNED_SIZE(_ZSTR_STRUCT_SIZE(buf_len)));
