...
static inline void link_block(sma_header_t *smaheader, block_t *cur) {
	...
	block_t *nxt = BLOCKAT(dst->fnext);

	/* Try to keep a big block at the beginning of the free list. This improves the performance
	 * of allocations and reduces the lock contention of the sma lock. */
